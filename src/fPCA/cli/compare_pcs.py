...
from fPCA.pc_similarity import PcSimilarity
from fPCA.peaks_similarity import PeaksSimilarity
from fPCA.utils import setup_logger, PCS
from fPCA.represent_pc import RepresentPC
from PCACorrelator.utils_classify_pcs import ClassifyPCSParser
...
class ComparePCS:
    ...
    def get_sub_name(self, i: int) -> str:
        ...
        if not hasattr(self, '_parser'):
            self._parser = ClassifyPCSParser(os.path.dirname(self.files_path[0]), self.movements)
        return self._parser.get_example_sub_name(os.path.basename(self.file_list[i]))

    def get_sub_mov(self, i: int) -> int:
        """Get the movement number of the i-th file (starts from 1)."""
        # Use same logic as ClassifyPCSParser
